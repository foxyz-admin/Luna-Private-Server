...
module.exports = async (req, res) => {
  ...
  if (!username || !password) return res.status(400).json({ error: 'Dados em falta.' });
  ...
  try {
    ...
    if (snap.exists()) return res.status(400).json({ error: 'Usuário existente.' });
    ...
    await ref.set({
      ...
      inventario: {
        temporada: "2018",
        passes: ["Flor de Sakura", "Hip Hop"],
        skins: ["Calça Angelical Original", "Top Criminal Verde"]
      }
    });
